...
public class VideoPanel extends JPanel implements IVideoListener, IMetadataListener, ComponentListener
{
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if (bufferedImage != null)
        {
            ...
            if (resized)
            {
                updateLetterBox(bufferedImage.getWidth(), bufferedImage.getHeight(), getWidth(), getHeight());
                resized = false;
            }
    ...
    public void onVideoReceived(VideoFrame frame)
    {
        ...
        if (bufferedImage == null || frame.getImage().getWidth() != bufferedImage.getWidth() ||
                frame.getImage().getHeight() != bufferedImage.getHeight() || resized)
        {
            resized = true;
        }
    ...
    private void updateLetterBox(int imageWidth, int imageHeight, int panelWidth, int panelHeight)
    {
        ...
        width = (panelAspect >= imageAspect) ? (int) Math.floor(imageAspect * panelHeight) : panelWidth;
        height = (panelAspect >= imageAspect) ? panelHeight : (int) Math.floor(panelWidth / imageAspect);
