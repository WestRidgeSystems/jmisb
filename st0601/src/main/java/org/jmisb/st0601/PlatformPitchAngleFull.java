 ...
 * <blockquote>
 *
 * Aircraft pitch angle. Angle between longitudinal axis and horizontal plane. Positive angles above
 * horizontal plane.
 *
...
public class PlatformPitchAngleFull implements IUasDatalinkValue {
    ...
    public PlatformPitchAngleFull(double degrees) {
        if (degrees != Double.POSITIVE_INFINITY && (degrees < -90 || degrees > 90)) {
            throw new IllegalArgumentException(
                    "Platform Pitch Angle (Full) must be in range [-90,90]");
        }
    ...
    public PlatformPitchAngleFull(byte[] bytes) {
        if (bytes.length != 4) {
            throw new IllegalArgumentException(
                    "Platform Pitch Angle (Full) encoding is a 4-byte signed int");
        }
