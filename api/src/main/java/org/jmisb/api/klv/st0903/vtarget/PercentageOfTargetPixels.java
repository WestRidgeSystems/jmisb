 ...
 * <blockquote>
 *
 * The percentage of pixels within the bounding box detected to be target pixels rather than
 * background pixels. Range 1 to 100, where 100 signifies that the target completely fills the
 * bounding box. Use of the VMask, VChip, VObject, or VFeature Local Sets recommended where more
 * detail about a target is necessary. If a detection has occurred, the size of the bounding box
 * should be such that a non-zero percentage of pixels overlaps the target.
 *
 * </blockquote>
 */
    ...
    public PercentageOfTargetPixels(short percentage) {
        if (percentage < MIN_VALUE || percentage > MAX_VALUE) {
            throw new IllegalArgumentException(
                    this.getDisplayName() + " value must be in range [1,100]");
        }
