 ...
 * <blockquote>
 *
 * Three or more points that represent the vertices of a polygon within a Motion Imagery Frame. List
 * the points in clockwise order. Close the polygon by connecting the last point to the first point.
 * Each point is a pixel number with numbering commencing with 1, at the top left pixel, proceeding
 * from left to right, top to bottom, then encoded using the Length-Value construct of a
 * Variable-Length Pack.
 *
 * <p>Note: in the UML of the VMTI LS the type of polygon is indicated as an array i.e., [].
 *
 * <p>The calculation of the pixel number is pixel number = Column + ((Row-1) x frame width)). The
 * top left pixel of the frame equates to (Column, Row) = (1, 1) and pixel number=1. For example,
 * for frame width = 1920, pixel location (1, 1) pixel number=1; pixel location (2, 1) pixel
 * number=2; pixel location (1, 2) pixel number=1921.
 *
 * </blockquote>
 */
public class PixelPolygon implements IVmtiMetadataValue {
    ...
    public PixelPolygon(byte[] bytes) {
        ...
        while (index < bytes.length - 1) {
            ...
            byte[] polygonPointBytes =
                    Arrays.copyOfRange(bytes, index, index + lengthField.getValue());
