 ...
 * Heading-style Angle.
 *
 * Used by ST 0601 tag 5 (Platform Heading Angle), tag 35 (Wind Direction),
 * tag 64 (Platform Magnetic Heading) and tag 71 (Alternate Platform Heading).
 * <p>
 * From ST:
 * <blockquote>
...
{
    protected static double RANGE = 360.0;
    protected static double MAXINT = 65535.0; // 2^16 - 1
    ...
    public UasDatalinkAngle360(byte[] bytes)
    {
        if (bytes.length != 2)
        {
            throw new IllegalArgumentException(this.getDisplayName() + " Platform Heading Angle encoding is a 2-byte unsigned int");
        }
