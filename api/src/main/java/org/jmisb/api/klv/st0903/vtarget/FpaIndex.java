 ...
 * From ST0903:
 * <blockquote>
 * Specifies the Focal Plane Array (FPA) in which detection of the target occurs
 * with FPA Row and FPA Column, in that order, in a two-dimensional array of
 * FPAs. The purpose of the FPA Index is to support sensors constructed using
 * multiple FPAs, such as Large Volume Motion Imagery (LVMI). Numbering for Rows
 * is from 1, top to bottom, starting from the top of the array. Numbering for
...
public class FpaIndex implements IVmtiMetadataValue
{
    ...
    public FpaIndex(FpaIndexPack fpaIndexPack)
    {
        ...
        {
            throw new IllegalArgumentException(this.getDisplayName() + " row must be in range [1,255]");
        }
        if ((fpaIndexPack.getFpaColumn() < 1) || (fpaIndexPack.getFpaColumn() > 255))
        {
            throw new IllegalArgumentException(this.getDisplayName() + " column must be in range [1,255]");
        }
        value = fpaIndexPack;
    }
    ...
    public FpaIndex(byte[] bytes)
    {
        ...
        {
            throw new IllegalArgumentException(this.getDisplayName() + " encoding is two byte pack");
        }
