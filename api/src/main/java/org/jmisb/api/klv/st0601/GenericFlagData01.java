...
/**
 * Generic Flag Data (ST 0601 tags 47).
 * <p>
 * From ST:
 * <blockquote>
 * Generic metadata flags.
 * <p>
 * The Generic Data Flags are miscellaneous boolean (yes / no) aircraft and
 * image related data settings which are individual bits in a single byte value.
 * </blockquote>
 * Initial versions of ST0601.13, ST0601.14, ST0601.15 and ST0601.16
 * inadvertently reversed the meaning of these bits. We use the original
 * (intended) meaning, not the interpretation published in those initial
 * versions.
 */
public class GenericFlagData01 implements IUasDatalinkValue, INestedKlvValue
{
     ...
     * Create from value.
     *
     * Each of the keys that is set in the parameter list will be set (1). The meaning of set is:
     * <ul>
     * <li>LaserRange: Laser is on.</li>
     * <li>AutoTrack: Auto-track is on.</li>
     * <li>IR_Polarity: Black hot.</li>
     * <li>IcingStatus: Icing detected.</li>
     * <li>SlantRange: Measured.</li>
     * </ul>
     *
    ...
    public GenericFlagData01(List<FlagDataKey> values)
    {
        ...
        {
            map.put(FlagDataKey.LaserRange, new UasDatalinkString(LASER_RANGE_STRING_KEY, LASER_ON));
        }
        else
        {
            map.put(FlagDataKey.LaserRange, new UasDatalinkString(LASER_RANGE_STRING_KEY, LASER_OFF));
        }

        if (values.contains(FlagDataKey.AutoTrack))
        {
            map.put(FlagDataKey.AutoTrack, new UasDatalinkString(AUTO_TRACK_STRING_KEY, AUTO_TRACK_ON));
        }
        else
        {
            map.put(FlagDataKey.AutoTrack, new UasDatalinkString(AUTO_TRACK_STRING_KEY, AUTO_TRACK_OFF));
        }

        if (values.contains(FlagDataKey.IR_Polarity))
        {
            map.put(FlagDataKey.IR_Polarity, new UasDatalinkString(IR_POLARITY_STRING_KEY, BLACK_HOT));
        }
        else
        {
            map.put(FlagDataKey.IR_Polarity, new UasDatalinkString(IR_POLARITY_STRING_KEY, WHITE_HOT));
        }

        if (values.contains(FlagDataKey.IcingStatus))
        {
            map.put(FlagDataKey.IcingStatus, new UasDatalinkString(ICING_STATUS_STRING_KEY, ICING_DETECTED));
        }
        else
        {
            map.put(FlagDataKey.IcingStatus, new UasDatalinkString(ICING_STATUS_STRING_KEY, NO_ICING_DETECTED));
        }

        if (values.contains(FlagDataKey.SlantRange))
        {
            map.put(FlagDataKey.SlantRange, new UasDatalinkString(SLANT_RANGE_STRING_KEY, MEASURED));
        }
        else
        {
            map.put(FlagDataKey.SlantRange, new UasDatalinkString(SLANT_RANGE_STRING_KEY, CALCULATED));
        }

        if (values.contains(FlagDataKey.ImageInvalid))
        {
            map.put(FlagDataKey.ImageInvalid, new UasDatalinkString(IMAGE_INVALID_STRING_KEY, IMAGE_INVALID));
        }
        else
        {
            map.put(FlagDataKey.ImageInvalid, new UasDatalinkString(IMAGE_INVALID_STRING_KEY, IMAGE_VALID));
        }
    }
