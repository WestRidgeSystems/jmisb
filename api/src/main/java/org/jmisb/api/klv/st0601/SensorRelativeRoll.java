 ...
 * From ST:
 * <blockquote>
 * Relative roll angle of sensor to aircraft platform. Twisting angle of camera about
 * lens axis. Top of image is zero degrees. Positive angles are clockwise when looking from
 * behind camera.
 * <p>
 ...
 * </blockquote>
 */
public class SensorRelativeRoll implements UasDatalinkValue
{
    ...
    public SensorRelativeRoll(double degrees)
    {
        ...
        {
            throw new IllegalArgumentException("Sensor Relative Roll angle must be in range [0,360]");
        }
    ...
    public SensorRelativeRoll(byte[] bytes)
    {
        if (bytes.length != 4)
        {
            throw new IllegalArgumentException("Sensor Relative Roll encoding is a 4-byte unsigned int");
        }
