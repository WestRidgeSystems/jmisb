 ...
 * <blockquote>
 *
 * Platform roll angle. Angle between transverse axis and transverse-longitudinal plane. Positive
 * angles for lowered right wing.
 *
 * <p>Map -(2^31-1)..(2^31-1) to +/-90. Use -(2^31) = 0x80000000 as an "error" indicator.
 *
...
public class PlatformRollAngleFull implements IUasDatalinkValue {
    ...
    public PlatformRollAngleFull(double degrees) {
        if (degrees != Double.POSITIVE_INFINITY && (degrees < -90 || degrees > 90)) {
            throw new IllegalArgumentException(
                    "Platform Roll Angle (Full) must be in range [-90,90]");
        }
    ...
    public PlatformRollAngleFull(byte[] bytes) {
        if (bytes.length != 4) {
            throw new IllegalArgumentException(
                    "Platform Roll Angle (Full) encoding is a 4-byte signed int");
        }
