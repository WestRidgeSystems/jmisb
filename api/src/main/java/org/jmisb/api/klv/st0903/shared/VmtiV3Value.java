...
/**
 * Shared implementation of the VMTI (ST0903) types that can hold unsigned integer variable length
...
public abstract class VmtiV3Value implements IVmtiMetadataValue {
    ...
    public VmtiV3Value(final int intValue) {
        if (intValue < MIN_VALUE || intValue > MAX_VALUE) {
            throw new IllegalArgumentException(
                    this.getDisplayName() + " value must be in range [0,16777215]");
        }
    ...
    public VmtiV3Value(final byte[] bytes) {
        if (bytes.length > 3) {
            throw new IllegalArgumentException(
                    this.getDisplayName() + " encoding is maximum three byte unsigned integer");
        }
        parse(bytes);
    }
