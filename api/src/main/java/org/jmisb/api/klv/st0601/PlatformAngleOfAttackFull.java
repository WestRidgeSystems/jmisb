 ...
 * From ST:
 * <blockquote>
 * Platform Attack Angle. Angle between platform longitudinal axis and relative wind.
...
public class PlatformAngleOfAttackFull implements IUasDatalinkValue
{
    ...
    public PlatformAngleOfAttackFull(double degrees)
    {
        ...
        {
            throw new IllegalArgumentException("Platform Angle of Attack (Full) must be in range [-90,90]");
        }
    ...
    public PlatformAngleOfAttackFull(byte[] bytes)
    {
        if (bytes.length != 4)
        {
            throw new IllegalArgumentException("Platform Angle of Attack (Full) encoding is a 4-byte signed int");
        }
