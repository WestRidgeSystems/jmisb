 ...
 * </blockquote>
 */
public class SensorRelativeElevation implements UasDatalinkValue
{
    ...
    /**
     * Create from value
     * @param degrees Sensor Relative Elevation, in degrees [-180,180], or {@code Double.POSITIVE_INFINITY}
     *                to represent "out of range"
     */
    public SensorRelativeElevation(double degrees)
    {
        if (degrees != Double.POSITIVE_INFINITY && (degrees < -180 || degrees > 180))
        {
            throw new IllegalArgumentException("Sensor Relative Elevation angle must be in range [-180,180]");
        }
    ...
    public SensorRelativeElevation(byte[] bytes)
    {
        if (bytes.length != 4)
        {
            throw new IllegalArgumentException("Sensor Relative Elevation encoding is a 4-byte int");
        }
