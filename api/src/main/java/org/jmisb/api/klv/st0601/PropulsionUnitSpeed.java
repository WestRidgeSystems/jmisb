 ...
 *
 * <p>Resolution: 1 revolution/minute.
 *
 * <p>RPMs can apply to combustion engine or electric motor propelling the aircraft.
 *
 * <p>With multi-rotor aircraft, use an average or other representative value.
 *
 * </blockquote>
 */
public class PropulsionUnitSpeed implements IUasDatalinkValue {
    ...
    public PropulsionUnitSpeed(long speed) {
        if (speed > MAX_VAL || speed < MIN_VAL) {
            throw new IllegalArgumentException(
                    this.getDisplayName() + " must be in range [0,2^32-1]");
        }
        this.speed = speed;
    }
    ...
    public PropulsionUnitSpeed(byte[] bytes) {
        if (bytes.length > MAX_BYTES) {
            throw new IllegalArgumentException(
                    this.getDisplayName() + " field length must be 1 - 4 bytes");
        }
