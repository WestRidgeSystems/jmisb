 ...
 * another system) the same identifier applies for that device (i.e.,
 * consistent). Physical Identifiers are frame accurate, meaning every frame of
 * the Motion Imagery Data associates with a Physical Identifier. No device will
 ...
 * change to the Virtual Identifier when swapping or changing the device.
 * Inserting a Virtual Identifier into all copies of the Motion Imagery Data
 * sent from, or stored on, the platform maintains the ubiquitous nature of the
 * identifier. Although a Virtual Identifier does not need to be frame accurate,
 * requirement MISB ST 1204.1-12 places limits on its update rate.
 * <p>
 * <b>Managed Identifier</b>: generated or stored by the control station (e.g.,
 * GCS) and managed by the control station so it is persistent. Devices in a
 * control station need to provide the exact same identifier (i.e., persistent)
 * if power cycled. The control station will manage changes to the Managed
 * Identifier when swapping or changing the device. Although a Managed
 * Identifier does not need to be frame accurate, requirement MISB ST 1204.1-16
 * places limits on its update rate.
 * <p>
 * The difference between a Virtual Identifier and Managed Identifier is the
 * Virtual Identifier is ubiquitous while a Managed Identifier only serves users
 * downstream from the control station; this means a Virtual Identifier will be
