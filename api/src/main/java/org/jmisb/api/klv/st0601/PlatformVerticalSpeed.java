 ...
 * <p>
 * Platform Vertical Speed is the climb or decent rate in meters per second of
 * an airborne platform in the zenith direction. Positive values indicate an
 * ascending platform, while negative value indicate descending.
 * <p>
 * Map (-2^15-1)..(2^15-1) to +/-180m/s. Use -(2^15) as "out of range" indicator. -(2^15) = 0x8000.
 * <p>
 * Resolution: ~0.0055 meters/second
 * </blockquote>
 */
public class PlatformVerticalSpeed implements IUasDatalinkValue
{
    ...
    /**
     * Create from value
     * @param speed The value in metres per second, or
     * {@code Double.POSITIVE_INFINITY} to represent "out of range"
     */
    public PlatformVerticalSpeed(final double speed)
    {
        if (speed != Double.POSITIVE_INFINITY && (speed < -180 || speed > 180))
        {
            throw new IllegalArgumentException(this.getDisplayName() + " must be in range [-180,180]");
        }
    ...
    public PlatformVerticalSpeed(final byte[] bytes)
    {
        if (bytes.length != 2)
        {
            throw new IllegalArgumentException(this.getDisplayName() + " encoding is a 2-byte signed int");
        }
