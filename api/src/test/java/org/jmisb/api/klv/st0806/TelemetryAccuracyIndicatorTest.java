...
public class TelemetryAccuracyIndicatorTest
{
    ...
    {
        TelemetryAccuracyIndicator version = new TelemetryAccuracyIndicator(new byte[]{(byte)0x04});
        assertEquals(version.getBytes(), new byte[]{(byte)0x04});
        assertEquals(version.getDisplayName(), "Telemetry Accuracy Indicator");
        assertEquals(version.getDisplayableValue(), "4");
        assertEquals(version.getValue(), 4);
    ...
    {
        TelemetryAccuracyIndicator version = new TelemetryAccuracyIndicator(new byte[]{(byte)0x00});
    ...
    {
        TelemetryAccuracyIndicator version = new TelemetryAccuracyIndicator(new byte[]{(byte)0xFF});
        assertEquals(version.getBytes(), new byte[]{(byte)0xFF});
        assertEquals(version.getDisplayName(), "Telemetry Accuracy Indicator");
        assertEquals(version.getDisplayableValue(), "255");
        assertEquals(version.getValue(), 255);
    ...
    {
        IRvtMetadataValue value = RvtLocalSet.createValue(RvtMetadataKey.TelemetryAccuracyIndicator, new byte[]{(byte)0x04});
