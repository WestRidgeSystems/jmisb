 ...
 *
 * The concept is that there is a test logger that should always contain no log
 * messages after a test has been run. That gets verified in the AfterMethod so
 * if there is a message, the logger needs to be checked and clear()ed before
 * the test method returns.
 *
 * Only ERROR, WARN and INFO levels are checked. DEBUG and lower are
 * implementation detail.
 *
 * The subclass is responsible for initialising the LOGGER correctly by calling
 * the super constructor with the class that is creating the log messages.
 */
abstract public class LoggerChecks
{
    ...
    protected void verifySingleLoggerMessage(final String expectedMessage)
    {
        ...
        String message = MessageFormatter.format(event.getMessage(), event.getArguments().get(0)).getMessage();
